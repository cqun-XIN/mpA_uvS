...
from layers.sta_attention import STA_SelfAttention
...
class MultiAttention(nn.Module):
    def __init__(self, input_size=1024, output_size=1024, freq=10000, pos_enc=None,
        ...
        if self.num_segments is not None:
            assert self.num_segments >= 2, "num_segments must be None or 2+"
            self.local_attention = nn.ModuleList()
            #self.sta_attention = nn.ModuleList()
            for _ in range(self.num_segments):
                # Local Attention, considering differences among the same segment with reduce hidden size
                self.local_attention.append(SelfAttention(input_size=input_size, output_size=output_size//num_segments,
                                                          freq=freq, pos_enc=pos_enc, heads=4))
            #spatial–temporal attention , considering differences among the same segment with reduce input_size and hidden size
            # for _ in range(self.num_segments*self.num_segments):
            #     self.sta_attention.append(STA_SelfAttention(input_size=input_size//num_segments, output_size=output_size//num_segments,
            #                                               freq=freq, pos_enc=pos_enc, heads=4))

            self.sta_attention = STA_SelfAttention(input_size=input_size, output_size=output_size,
                                  freq=freq, pos_enc=pos_enc, heads=1, num_segments=4)
    ...
    def forward(self, x):
        ...
        if self.num_segments is not None and self.fusion is not None:
            ...
            weighted_sta_value = self.sta_attention(x)  #sta attention
            # Normalize the features vectors
            weighted_sta_value = F.normalize(weighted_sta_value, p=2, dim=1)
            if self.fusion == "add":
                weighted_value += weighted_sta_value
            elif self.fusion == "mult":
                weighted_value *= weighted_sta_value
            elif self.fusion == "avg":
                weighted_value += weighted_sta_value
                ...
                weighted_value = torch.max(weighted_value.clone(),
                                           weighted_sta_value)
            #weighted_value += weighted_sta_value
